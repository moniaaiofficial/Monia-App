...
export default function ChatsPage() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      const { data, error } = await supabase.rpc('search_users', { search_term: profileQuery.trim() });
      if (error) {
          ...
          {!profileSearching && profileResults.map((p) => (
            <button key={p.id} onClick={() => startChat(p.id)}
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <p style={{ color: 'rgba(255,255,255,0.38)', fontSize: 12, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.username}</p>
              </div>
            </button>
          ))}
