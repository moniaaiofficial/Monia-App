    ...
    <div style={{ position: 'fixed', bottom: 88, left: 0, right: 0, zIndex: 50, padding: '14px 16px', background: 'rgba(6,0,12,0.96)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
      {!hasRecording ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ flex: 1, height: 3, borderRadius: 2, background: 'rgba(255,255,255,0.08)', overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${(elapsed / MAX_SECONDS) * 100}%`, background: '#ff0066', transition: 'width 1s linear' }} />
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <button
            onClick={handleSend}
            style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg,#ff0066,#a8e000)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer', boxShadow: '0 0 16px rgba(198,255,51,0.35)', flexShrink: 0 }}
          >
            <Send size={18} style={{ color: '#14141f', marginLeft: 2 }} />
          </button>
        </div>
      )}
    </div>
